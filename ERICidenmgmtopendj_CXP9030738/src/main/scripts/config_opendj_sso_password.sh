...
#
###############################################################################
# This script is used to change the OpenDJ sso_user password
...
SSO_PASSKEY=/ericsson/tor/data/idenmgmt/ssoldap_passkey

# set logs file
LOG_FILE="/var/log/opendj/opendj-sso-password-change-`/bin/date "+%F:%H:%M:%S%:z"`.log"
...
echo "Script to change COM_INF_LDAP_ADMIN_ACCESS"
...
read -r CURRENT_PASSKEY < $SSO_PASSKEY
...
# Verify old password
CheckPassword "COM_INF_LDAP_ADMIN_ACCESS" $SSO_PASSKEY
if [ $? != 0 ] ; then
...
# Modify OpenDJ sso_user password
StorePasswordinOpenDJ "$COM_INF_LDAP_ADMIN_CN" "$NEW_PASS"
if [ $? != 0 ] ; then
